...
export default class SortableGrid extends Component {
  ...
  onMoveBlock = (evt, {moveX, moveY, dx, dy}) => {
    if (this.state.activeBlock != null && this._blockPositionsSet()) {
      ...
      if (closest !== this.state.activeBlock && !(this.items[closest].props.inactive === true)) {
        Animated.timing(
  ...
  assessGridSize = ({nativeEvent}) => {
    ...
    if (this.props.itemWidth && this.props.itemWidth < nativeEvent.layout.width) {
      ...
      this.blockHeight = this.props.itemHeight
    }
  ...
  _getDeletionView = (key) => {
    if (this.state.deleteModeOn)
      return <Image style={ this._getImageDeleteIconStyle(key) } source={require('../../assets/images/trash.png')} />
  }
